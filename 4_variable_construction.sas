...
*     where q  =  (MVE + PS + DEBT) / AT. Specifically, MVE is the product of a firm's share price (item 199) and the
*     number of common stock shares outstanding (item 25), PS is the liquidating value of the firm's outstanding preferred
*     stock (item 10), DEBT is the value of the firm's short term liabilities (item 9) net of its short-term assets (item 4), 
*     plus the book value of the firm's long-term debt (item 34), and TA is book value of total assets (item 6).
* <5> Firm size: FS (2006) use natural log of sales, natural log of capital, and natural log of assets for proxies. 
* <6> Firm age: According to Strebulaev & Yang (2013), firm age is defined as the number of years in Compustat. We will
*     take natural log of the age. 
*
* Overall, we need item 13,2,3,72,68,6 for ROA, item 237,60,74 for ROE, item 12,6 for sales over assets,
* item 199,25,10,9,4,34,6 for Tobin's q, item 6 for firm size (the exact item name corresponds to item number can be 
...
data data;
	set data (keep=gvkey datadate fyear fyr aco at ceq csho ibcom invt lco oibdp rect sale
			  txdb sich prcc_f pstkl dltt lct act);
...
data data(drop=prev_fyear prev_rect prev_invt prev_lco prev_aco);
	...
	if first.gvkey then do; /*no previous year for calculating the change*/
        drect = .;
        dinvt = .;
        ilco  = .;
        daco  = .;
    end;
	else if fyear = prev_fyear + 1 then do; /*calculate the change only if there is previous year*/
	...
	if at > 0 then do;
	    ...
	    tobinsq = (csho * prcc_f + pstkl + dltt + lct - act) / at;
		size = log(at);
	end;
	else do;
	    roa = .;
...
proc sql;
	...
	select a.*, b.first_fyear, log((a.fyear - b.first_fyear + 1)) as age
	from data_1950 as a 
...
******************************************************************
Part 5 : Export the merged data using .csv file
******************************************************************;
proc export data=regression_data 
    outfile="C:\Users\¤ý«FÒj\Desktop\RA\Cha\Data\regression_data.csv"
    dbms=csv
    replace;
run;	